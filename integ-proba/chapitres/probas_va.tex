...
\begin{document}
    ...
    \begin{itemize}
        \itemb un espace probabilisé $\left( \Omega, \A, \Pb \right)$ ;
        \itemb $n$ variables aléatoires réelles ou discrètes $\left( X_i \right)_{i \in [\![1, n]\!]}$, que l'on peut voir comme un vecteur aléatoire $X$ à $n$ composantes.
    \end{itemize}
    ...
    \begin{rem}
        On pourrait évidemment introduire la notion de loi jointe et de loi marginale dans le cas de variables aléatoires quelconques.
        Cependant, en pratique ce n'est que très peu utilisé et cela ne permet de pas de se rapprocher de la notion de vecteur aléatoire (puisque ses composantes doivent être des variables aléatoires réelles ou discrètes).
    \end{rem}

    \begin{rem}
        \textbf{La connaissance de la loi jointe permet de connaître toutes les lois marginales}.
        En effet, il suffit d'écrire :
        \begin{displaymath}
            \begin{array}{rcl}
                \forall A \in \B(\R), \Pb_{X_{i_0}}(A) & = & \Pb(X_{i_0}^{-1}(A)) \\
                                                   & = & \Pb(X_1^{-1}(\R) \cap \ldots \cap X_{i_0}^{-1}(A) \cap \ldots \cap X_n^{-1}(\R)) \\
                                                   & = & \Pb(X^{-1}(\R \times \ldots \times A \times \ldots \times \R) \\
                                                   & = & \Pb_X(\R \times \ldots \times A \times \ldots \times \R)
            \end{array}
        \end{displaymath}
        ...
        Il est donc impensable de pouvoir connaître la loi du vecteur aléatoire résultant.
    \end{rem}

    \begin{prop}
        Supposons que $X$ soit un vecteur aléatoire à densité $f$.
        Alors les lois marginales sont à densité (\textit{ie} les $\left( X_i \right)_{i \in [\![1, n]\!]}$ sont des variables aléatoires à densité), de densité $\left(f_i\right)_{i \in [\![1, n]\!]}$, et pour tout i $\in [\![1, n]\!]$ :
        \begin{displaymath}
    ...
    \begin{defi}
        Soit $X$ et $Y$ deux variables aléatoires quelconques à valeurs dans $E$.
        $X$ et $Y$ sont dites \textbf{indépendantes} lorsque l'une des deux conditions équivalentes est remplie :
        \begin{itemize}
            \itemb la loi jointe de $X$ et $Y$ $\Pb_{(X,Y)}$ vérifie $\Pb_{(X,Y)} = \Pb_X \otimes \Pb_Y$ ;
            \itemb pour tout $(A,B) \in \B(E)^2$, $\left\{ X \in A \right\}$ et $\left\{ Y \in B \right\}$ sont deux évènements indépendants.
        \end{itemize}
    \end{defi}
