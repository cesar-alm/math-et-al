...
\newtheorem{defi}{Définiton}[section]
...
Plus généralement, en notant $\lambda$ l'application qui à un segment $\left[a,b\right]$ associe sa longueur $b-a$ pourrait s'appeler dans le langage courant une \textit{mesure}, c'est-à-dire une fonction permettant d'obtenir la mesure (la taille) d'un objet.
...
\begin{exemple}
  La \textbf{mesure de Lebesgue} sur $\mathbb{R}$ est la mesure qui prolonge la notion naturelle de longueur.
  Elle est habituellement notée $\lambda$.

  La tribu sur $\mathbb{R}$ permettant d'utiliser cette mesure est la \textbf{tribu de Lesbesgue}.
  Sa construction est trop complexe pour être exposée ici.
  Néanmoins, le lecteur pourra se souvenir que la tribu de Lebesgue est une complétion de la \textbf{tribu de Borel} (présentée dans l'exemple \ref{ex:borel}).
  La notion de complétion de mesure est détaillée dans la remarque \ref{rem:completion}.

  Cette définition s'adapte immédiatement si l'on se place dans $\mathbb{R}^n$.
  La mesure de Lebesgue prolonge alors la notion d'aire si $n=2$, de volume si $n=3$, etc.
\end{exemple}
